module EmsCommon
  ...
  def show
    ...
    elsif @display == "middleware_servers" || session[:display] == "middleware_servers" && params[:display].nil?
      title = ui_lookup(:tables => "middleware_servers")
      drop_breadcrumb(:name => @ems.name + " (All #{title})",
                      :url  => show_link(@ems, :display => @display))
      @view, @pages = get_view(MiddlewareServer, :parent => @ems)  # Get the records (into a view) and the paginator
      @showtype = @display
      if @view.extras[:total_count] > @view.extras[:auth_count] && @view.extras[:total_count] &&
          @view.extras[:auth_count]
        @bottom_msg = "* You are not authorized to view " +
